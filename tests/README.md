...
| `timer_and_sync` | 80 | Rest timer accuracy under a stalled clock, the screen wake lock, saving sets and finished workouts through a network drop, a workout the server refuses not holding up the ones queued behind it, a reload mid-workout on a connection that stalls instead of failing, the server's copy of the workout still winning when it changed on another device, two tabs uploading the same queue, and 30 seconds more or less rest, running or paused, down to a quiet stop at zero |
