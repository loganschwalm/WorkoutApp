...
| `gym_usability` | 103 | Last time's numbers, correcting logged sets, moving between exercises, skipped exercises, message visibility, the phone layout, session expiry, sign-in links crafted to lead off-site afterwards, signing out, and the controls within reach (the set entry first, the −5/+5 buttons, the plates for barbell lifts, the rest timer staying on screen, notes folded away) |
